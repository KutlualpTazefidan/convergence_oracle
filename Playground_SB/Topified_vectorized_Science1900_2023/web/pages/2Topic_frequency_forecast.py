...
def plot_bertopic_data_monthly_freq(selected_topic,future_months, topic_mapping):
    ...
    plt.title(f'Topic {target_topic} Topic Frequency Forecast (RMSE: {rmse:.2f})')
    plt.legend()
    plt.grid(True)
...
def plot_bertopic_data_yearly_freq(selected_topic,future_years, topic_mapping):
    ...
    grouped_df = aws1_dfs.groupby(['topic_code', 'year'])['ones_column'].sum().reset_index()
    
    ## for sum 
    ## grouped_df = aws1_dfs.groupby(['topic_code', 'year'])[type_of_citation].count().reset_index()


    pivot_df = grouped_df.pivot(index='year', columns='topic_code', values='ones_column')
...
# Streamlit app title
st.title("Topic frequency prediction")
...
if selected_topic in aws_dfs['topic_code'].unique():
    ...
    # this section to create a button to trigger the prediction
if st.button("Predict"):
    ...
    if selected_model in model_paths:
        if selected_model == 'monthly_groupby':
            plot_bertopic_data_monthly_freq(selected_topic_code, future_months, topic_mapping)
        elif selected_model == 'yearly_groupby':
            plot_bertopic_data_yearly_freq(selected_topic_code, future_years, topic_mapping)
    else:
